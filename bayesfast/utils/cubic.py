...
class cubic_spline:
    """
    Cubic spline data interpolator for 1-d monotonically increasing functions.

    This assumes the target function increases monotonically. Whenever it finds the interpolating
    function not monotonic in some interval, it will try to allocate more points in that interval.
    """
    __slots__ = ('_x', '_n', '_fun', '_c', '_y', '_mask')

    def __init__(self, x_all, fun, bins=100, edge_bins=1, edge_points=10, max_width=5, split=4,
                 max_add=5, save_fun=False):
        ...
        x_edge_1 = np.percentile(x_all[x_all < self._x[edge_bins]] - self._x[0], np.linspace(0, 100,
                                 edge_points + 2)[1:-1])
        y_edge_1 = fun(x_edge_1 + self._x[0]) - self._y[0]
        k_edge_1 = np.sum(x_edge_1 * y_edge_1) / np.sum(x_edge_1 * x_edge_1)
        x_edge_2 = np.percentile(x_all[x_all > self._x[-edge_bins - 1]] - self._x[-1],
        ...
        if i_1 <= i_2:
            ...
            if sparse_index.size:
                ...
                for j in sparse_index:
                    ...
                    x_aug_j = np.linspace(self._x[j], self._x[j + 1], n_j + 1)[1:-1]
        ...
        while (not np.all(check)) and add_points < max_add:
            ...
            for j in np.where(check == 0)[0]:
                x_aug_j = np.linspace(self._x[j], self._x[j + 1], split + 1)[1:-1]
                x_aug = np.concatenate((x_aug, x_aug_j))
            insert_index = np.searchsorted(self._x, x_aug)
            self._x = np.insert(self._x, insert_index, x_aug)
            self._y = np.insert(self._y, insert_index, fun(x_aug))
        ...
        if not np.all(check):
            ...
            for i_b in bad_index:
                self._c[i_b, 0] = 0
                self._c[i_b, 1] = 0
                self._c[i_b, 2] = (self._y[i_b] - self._y[i_b - 1]) / (self._x[i_b] -
                                                                       self._x[i_b - 1])
    ...
    def _regularize_y(self):
        ...
        while n_b > 0:
            while n_b > 0:
                ...
                k_b = (self._y[end_b + 1] - self._y[start_b]) / (self._x[end_b + 1] -
                                                                 self._x[start_b])
                for j_b in range(start_b + 1, end_b + 1):
                    self._y[j_b] = self._y[start_b] + k_b * (self._x[j_b] - self._x[start_b])
